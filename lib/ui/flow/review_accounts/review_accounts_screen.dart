...
import 'package:oriflame/ui/components/app_text_field.dart';
import 'package:oriflame/ui/components/place_holder_screen.dart';
...
import 'detail/review_account_detail.dart';
...
class _ReviewAccountsScreenState extends ConsumerState<ReviewAccountsScreen> {
  void _observeError(BuildContext context) {
    ref.listen(reviewAccountsStateNotifier.select((value) => value.error), (
      previous,
      next,
    ) {
      if (next != null) {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        const _ReviewAccountHeader(),
        const Divider(height: 1),
        FadeInSwitcher(child: const _ReviewAccountsActions()),
        Expanded(child: FadeInSwitcher(child: const _ReviewAccountsList())),
      ],
    );
  }
}

class _ReviewAccountsList extends ConsumerWidget {
  const _ReviewAccountsList();

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final state = ref.watch(
      reviewAccountsStateNotifier.select(
        (value) => (
          loading: value.loading,
          accounts: value.accounts,
          sortedAccounts: value.sortedAccounts,
        ),
      ),
    );
    ...
    if (state.accounts.isEmpty) {
      return PlaceHolderScreen(
        ...
        title: context.l10n.empty_review_accounts_title,
        description: context.l10n.empty_review_accounts_description,
      );
    }
          ...
          children: List.generate(
            ...
            (index) => AccountCard(
              ...
              onTap: () {
                showDialog(
                  context: context,
                  builder: (context) => ReviewAccountDetailDialog(
                    accounts: state.sortedAccounts,
                    index: index,
                  ),
                );
              },
...
class _ReviewAccountHeader extends StatelessWidget {
  const _ReviewAccountHeader();
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Row(
        children: [
          Text(
            context.l10n.review_accounts_title,
...
class _ReviewAccountsActions extends ConsumerStatefulWidget {
  const _ReviewAccountsActions();

  @override
  ConsumerState createState() => __ReviewAccountsActionsState();
}

class __ReviewAccountsActionsState
    extends ConsumerState<_ReviewAccountsActions> {
  final GlobalKey _menuKey = GlobalKey();
  late ReviewAccountsStateNotifier _notifier;

  @override
  void initState() {
    _notifier = ref.read(reviewAccountsStateNotifier.notifier);
    super.initState();
  }

  @override
  Widget build(BuildContext context) {
    final state = ref.watch(reviewAccountsStateNotifier);
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  OnTapScale(
                    onTap: () {
                      ArchiveAccountsRoute().go(context);
                    },
                    child: Container(
                      padding: EdgeInsets.symmetric(
                        horizontal: 16,
